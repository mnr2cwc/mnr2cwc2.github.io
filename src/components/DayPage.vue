  ...
  <div class="day-page" v-if="day">
    ...
    <Teleport to="body">
      <div v-if="lightboxOpen" class="lightbox" @click.self="closeLightbox">
        ...
        <button class="lightbox-prev" @click="prevPhoto" :disabled="lightboxIndex <= 1">‹</button>
        <div class="lightbox-content">
          <div class="lightbox-placeholder">
            <svg viewBox="0 0 80 60" fill="none" class="lb-icon">
              <rect x="2" y="2" width="76" height="56" rx="4" stroke="currentColor" stroke-width="1.5"/>
              <circle cx="24" cy="22" r="8" stroke="currentColor" stroke-width="1.5"/>
              <path d="M2 50L20 32L34 46L50 28L78 50" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/>
            </svg>
            <p>Photo {{ lightboxIndex }}</p>
            <p class="lb-sub">{{ day.label }} · {{ day.location }}</p>
          </div>
        </div>
        <button class="lightbox-next" @click="nextPhoto" :disabled="lightboxIndex >= 10">›</button>
      </div>
    </Teleport>
  </div>
</template>
...
.day-banner::before {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(ellipse at 70% 50%, rgba(196, 98, 45, 0.12) 0%, transparent 65%);
  pointer-events: none;
}
...
.photo-overlay {
  ...
  background: linear-gradient(to top, rgba(26,21,16,0.45) 0%, transparent 100%);
  opacity: 0;
